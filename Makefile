...
deps:
	...
	@if [ ! -f "$(DEPS_DIR)/nlohmann/json.hpp" ]; then \
		printf "$(YELLOW)Downloading JSON library with wget...$(RESET)\n"; \
		mkdir -p $(DEPS_DIR)/nlohmann; \
		wget -O $(DEPS_DIR)/nlohmann/json.hpp https://github.com/nlohmann/json/releases/download/v3.11.2/json.hpp; \
	fi
